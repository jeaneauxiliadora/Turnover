...
classificador = Pipeline(steps=[
    ("Classificador", DecisionTreeClassifier(criterion='entropy', max_depth=5, min_samples_split=2, min_samples_leaf=15, splitter='random', random_state=123143))
])
...
metrics_df.to_csv('ML/result/metrics_ct.csv', index=False)
...
roc_data.to_csv('ML/result/roc_ct_data.csv', index=False)
