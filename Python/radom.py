...
classificador = Pipeline(steps=[
    ("Classificador", RandomForestClassifier(n_estimators=300,criterion='gini', min_samples_split=5, max_depth=15,min_samples_leaf=3,  bootstrap=False, random_state=123143)
)
])
...
metrics_df.to_csv('ML/result/metrics_rf.csv', index=False)
...
roc_data.to_csv('ML/result/roc_rf_data.csv', index=False)
